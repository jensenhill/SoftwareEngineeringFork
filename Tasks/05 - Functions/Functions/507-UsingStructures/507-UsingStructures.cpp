...
int main()
{
    ...
    Point newbie = addPoint(p1, p2);
    printf("Newbie x = %d and y = %d and Length = %f", newbie.x, newbie.y, newbie.length);
}
...
Point addPoint(Point p1, Point p2)
{
    ...
    result.x = p1.x + p2.x;
    result.y = p1.y + p2.y;
    result.length = sqrt(result.x * result.x + result.y * result.y);
